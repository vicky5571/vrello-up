...
export function MousView() {
  ...
  // store. DELETE_MOU is admin-only, so only admins may delete.
  ...
  const fetchMous = useCallback(async () => {
    ...
    try {
      ...
      setMous(Array.isArray(json.data) ? json.data : []);
    } catch (e) {
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      if (deleted < selectedRowIds.length) {
        toast.error(`${deleted}/${selectedRowIds.length} MOUs deleted`);
      } else {
        toast.success(`${deleted} ${deleted === 1 ? "MOU" : "MOUs"} deleted`);
      }

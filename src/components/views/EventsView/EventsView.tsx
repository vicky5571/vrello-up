...
// Mirrors GET /api/marcom/events rows: MarcomEvent rows include
// their footage via the relation include.
...
const columnHelper = createColumnHelper<typeof features, MarcomEvent>();
...
export function EventsView() {
        ...
        columnHelper.display({
          ...
          cell: ({ row }) => (
            ...
            >
              <input
                type="checkbox"
                aria-label={`Select event ${row.original.name}`}
          ...
          cell: ({ row }) => (
            <span
              className={cn(
                ...
                STATUS_STYLES[row.original.status] ?? STATUS_STYLES.UPCOMING,
              )}
            >
  ...
  // Events are not tasks: row click toggles a local expandable detail row.
  ...
  const handleBulkDelete = async () => {
    ...
    if (!canManage) {
      toast.error("Only workspace admins or staff can delete events");
      return;
    }
    if (
      typeof window !== "undefined" &&
      !window.confirm(
        `Permanently delete ${selectedRowIds.length} ${selectedRowIds.length === 1 ? "event" : "events"}? This cannot be undone.`,
      )
    )
      return;
    setIsDeleting(true);
    try {
      const results = await Promise.all(
        selectedRowIds.map(async (id) => {
          const res = await fetch(`/api/marcom/events/${id}`, {
            method: "DELETE",
          });
          return res.ok;
        }),
      ...
      if (failed.length === 0) {
        toast.success(`${succeeded.length} ${succeeded.length === 1 ? "event" : "events"} deleted`);
      } else if (succeeded.length === 0) {
        toast.error(`Failed to delete ${failed.length} ${failed.length === 1 ? "event" : "events"}`);
      } else {
        toast.error(`${succeeded.length}/${selectedRowIds.length} events deleted — ${failed.length} failed`);
      }
  ...
  return (
    <div className="flex-1 overflow-auto p-6">
        ...
        <div className="flex items-center gap-2 bg-blue-50 dark:bg-blue-950/30 border border-blue-200 dark:border-blue-800/50 px-3 py-1.5 rounded-lg text-xs animate-in fade-in slide-in-from-top-1 duration-150 mb-3">
          ...
          {canManage ? (
            ...
            <span className="text-slate-400 dark:text-slate-500 px-2 py-0.5">
              Delete requires admin or staff role
            </span>
          )}
      ...
      <div className="rounded-lg border border-slate-200/90 dark:border-slate-800 bg-white dark:bg-[#18191B] shadow-2xs overflow-x-auto">
        <div style={{ minWidth: `${table.getTotalSize()}px` }} role="table" aria-label="Events">
          {/* Table Header Row */}
                      ...
                      <div className="px-4 py-3 bg-slate-50/60 dark:bg-slate-800/30 border-t border-slate-100 dark:border-slate-800/60">
                        <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-xs">
                          ...
                          <div>
                            ...
                            <div className="text-slate-700 dark:text-slate-300">
                              {event.attendeeCount}/{event.targetAttendee}
                            </div>
                          </div>

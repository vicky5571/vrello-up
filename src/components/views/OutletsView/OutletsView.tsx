...
export function OutletsView() {
  ...
  const handleBulkDelete = async () => {
    ...
    if (
      ...
      !window.confirm(
        `Permanently delete ${selectedRowIds.length} ${selectedRowIds.length === 1 ? "outlet" : "outlets"}? This cannot be undone.`,
      )
    )
      return;
    setIsDeleting(true);
    try {
      ...
      if (failed.length === 0) {
        toast.success(`${succeeded.length} ${succeeded.length === 1 ? "outlet" : "outlets"} deleted`);
      } else if (succeeded.length === 0) {
        toast.error(`Failed to delete ${failed.length} ${failed.length === 1 ? "outlet" : "outlets"}`);
      } else {
        toast.error(`${succeeded.length}/${selectedRowIds.length} outlets deleted — ${failed.length} failed`);
      }
  ...
  return (
    <div className="flex-1 overflow-auto p-6">
        ...
        <div className="flex items-center gap-2">
          {canManage && (
            <button
              ...
              className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold text-white bg-orange-600 hover:bg-orange-700 transition-colors shadow-2xs cursor-pointer"
            >
      ...
      <div className="rounded-lg border border-slate-200/90 dark:border-slate-800 bg-white dark:bg-[#18191B] shadow-2xs overflow-x-auto">
        <div style={{ minWidth: `${table.getTotalSize()}px` }} role="table" aria-label="Outlets">
          {/* Table Header Row */}
                ...
                return (
                  <div key={row.id}>
                    <div
                      ...
                      aria-controls={`outlet-detail-${outlet.id}`}
                      className={cn(
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setModalOutlet({ ...modalOutlet, type: e.target.value as OutletType })}
                    className="w-full px-3 py-1.5 text-xs rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 focus:outline-hidden focus:ring-2 focus:ring-orange-500 cursor-pointer"
                  >
                ...
                <div>
                  ...
                  <select
                    ...
                    onChange={(e) => setModalOutlet({ ...modalOutlet, tier: e.target.value as OutletTier })}
                    className="w-full px-3 py-1.5 text-xs rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-900 dark:text-slate-100 focus:outline-hidden focus:ring-2 focus:ring-orange-500 cursor-pointer"
                  >
